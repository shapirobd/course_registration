...
def get_semester_details(semester_ids):
    ...
    cursor.execute(f"SELECT id, season, year FROM semesters WHERE id IN ({','.join(map(str, semester_ids))}) ORDER BY id")
...
def get_major(student_id):
    ...
    cursor.execute(f"SELECT m.* FROM majors m INNER JOIN students s ON m.id = s.major_id WHERE s.id = {student_id} LIMIT 1")
...
def get_registered_courses(next_semester_id, student_id):
    ...
    cursor.execute("SELECT c.*, s.season, s.year FROM courses c "
                   "INNER JOIN student_courses stc ON c.id = stc.course_id "
                   "INNER JOIN semesters s ON s.id = stc.semester_id "
                   "WHERE stc.student_id=%(student_id)s AND s.id IN (%(next_semester_id)s - 1, %(next_semester_id)s)", {'student_id': student_id, 'next_semester_id': next_semester_id})
...
def get_completed_credits(next_semester_id, student_id):
    ...
    cursor.execute("SELECT c.id, c.course_type, sc.credits_earned AS credits FROM student_courses sc INNER JOIN courses c ON c.id = sc.course_id WHERE sc.student_id = %(student_id)s AND sc.semester_id < %(next_semester_id)s - 1 AND sc.credits_earned > 0 UNION "
                   "SELECT c.id, c.course_type, c.credits FROM student_courses sc INNER JOIN courses c ON c.id = sc.course_id WHERE student_id = %(student_id)s AND semester_id >= %(next_semester_id)s - 1", 
...
def register_for_course(semester_id, student_id, course_id):
    ...
    try:
      ...
      cursor.execute("SELECT * FROM courses WHERE id = %(course_id)s", {'course_id': course_id})
